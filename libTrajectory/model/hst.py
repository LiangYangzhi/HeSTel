...
class HST(torch.nn.Module):
    def __init__(self, in_dim, out_dim, heads, add_self_loops=True):
        super(HST, self).__init__()
        logging.info(f"add_self_loops={add_self_loops}")
        self.conv1 = HSTConv(
            in_channels=in_dim, out_channels=out_dim, edge_dim=1, add_self_loops=add_self_loops,
            heads=heads, dropout=0.2)
        self.conv2 = HSTConv(
            in_channels=out_dim*heads, out_channels=out_dim, edge_dim=1, add_self_loops=add_self_loops,
            heads=heads, dropout=0.2)
