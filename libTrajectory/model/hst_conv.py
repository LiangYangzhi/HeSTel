...
class HSTConv(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, PairTensor], edge_index: Adj, edge_attr: OptTensor = None,
                return_attention_weights=None, global_spatial=None, global_temporal=None,):
        ...
        if isinstance(x, Tensor):
            if x.shape[0] != self.heads * self.out_channels:
                x = x[:, :self.heads * self.out_channels]
            x: PairTensor = (x, x)
        ...
        if self.concat:
            ...
            out = out * global_spatial.unsqueeze(1) * global_temporal.unsqueeze(1)
        else:
            out = out.mean(dim=1)
        out = out
