...
class Executor(object):
    ...
    def train(self, train_tid, enhance_ns, test_tid=None, epoch_num=1, batch_size=64, num_workers=16):
        ...
        # logging.info(f"loss2 and loss3: top_loss penalty = {penalty2} ")
        ...
        for epoch in range(epoch_num):  # 每个epoch循环
            ...
            for (node1, edge_ind1, edge_attr1, node2, edge_ind2, edge_attr2,
                ...
                # sim1 = torch.matmul(tid1_vec, ns2_vec.T)
                # sim2 = torch.matmul(tid1_vec, ns1_vec.T)
                # diagonal = torch.diag(sim)
                # tid1_top = torch.cat([diagonal.unsqueeze(1), sim1], dim=1)
                # tid2_top = torch.cat([diagonal.unsqueeze(1), sim2], dim=1)
                # loss2 = top_loss(tid1_top, penalty=penalty2)
                # loss3 = top_loss(tid2_top, penalty=penalty2)
            ...
            for struct in data_loader:
                for data in struct:
                    ...
                    if 'cuda' in str(self.device):
                        ...
                        edge_ind2 = edge_ind2.to(device=self.device)
                        edge_attr2 = edge_attr2.to(device=self.device)
                    # x1 = net1(node1, edge_ind1, edge_attr1)
                    # x2 = net1(node2, edge_ind2, edge_attr2)
                    net1_x = net1(node1, edge_ind1, edge_attr1)
                    net2_x = net2(node2, edge_ind2, edge_attr2)
                    x1 = net3(net1_x, edge_ind1, edge_attr1)
                    x2 = net3(net2_x, edge_ind2, edge_attr2)
                    ...
                    # embedding_1.append(node1[0])
                    # embedding_2.append(node2[0])

            embedding_1 = np.array(embedding_1)
            embedding_2 = np.array(embedding_2)  # .astype(np.float32)
