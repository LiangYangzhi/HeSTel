...
class BLExecutor(object):
    ...
    def signature(self, config):
        ...
        for k, v in test_data.items():
            logging.info(f"{k}")
            v1, v2 = v
            _, _, acc = knn_evaluator(v1, v2)
            result[k] = acc
        with open(f'{self.log_path}bl_signature_sequential.json', 'w') as fp:
            json.dump(result, fp)
        ...
        for k, v in test_data.items():
            logging.info(f"{k}")
            v1, v2 = v
            _, _, acc = knn_evaluator(v1, v2, method=temporal_config['method'])
            result[k] = acc
        with open(f'{self.log_path}bl_signature_temporal.json', 'w') as fp:
            json.dump(result, fp)
        ...
        for k, v in test_data.items():
            ...
            _, _, acc = knn_evaluator(v1, v2)
            result[k] = acc
        with open(f'{self.log_path}bl_signature_spatial.json', 'w') as fp:
            json.dump(result, fp)
        ...
        for k, v in test_data.items():
            ...
            _, _, acc = knn_evaluator(v1, v2)
            result[k] = acc
        with open(f'{self.log_path}bl_signature_spatiotemporal.json', 'w') as fp:
            json.dump(result, fp)
    ...
    def gcn_infer(self, test_data):
        ...
        for k, v in test_data.items():  # file_name: tid
            ...
            _, _, acc = faiss_evaluator(embedding_1, embedding_2)
            result[k] = acc
        with open(f'{self.log_path}{net_name}.json', 'w') as fp:
            json.dump(result, fp)
