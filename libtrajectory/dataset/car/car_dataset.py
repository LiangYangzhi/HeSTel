...
class CarDataset(object):
    ...
    def _map(self):
        ...
        for doc in cursor:
            if "placeId" in doc:
                device_place[doc["cameraCode"]] = doc["placeId"]
            device_lon[doc["cameraCode"]] = doc["longitude"]
            device_lat[doc["cameraCode"]] = doc["latitude"]
        cursor.close()
    ...
    def _car(self):
        ...
        device_place, device_long, device_lat = self._map()
        for day in range(days):
            ...
            face["longitude"] = face["deviceId"].map(device_long)
            face["latitude"] = face["deviceId"].map(device_lat)
            face["placeId"] = face["deviceId"].map(device_place)
            if self.columns:
                face = face[self.columns]

            path = f"./libtrajectory/dataset/car/{self.city}/{loop_start_str}.csv"
    ...
    def _binging_device(self, confing):
        client = MongoClient(confing.get("client"))
        db = client[confing.get("db")]
        collection = db[confing.get("collection")]
        cursor = collection.find(confing.get("condition", {}))
        binging = []
        for document in cursor:
            binging.append(document['cameraCode'])
        return binging
    ...
    def _device(self):
        ...
        if self.device.get("binging", None):
            binging = self._binging_device(self.device.get("binging"))
            device['binging'] = device["deviceId"].map(lambda i: 1 if i in binging else 0)

        path = f"./libtrajectory/dataset/car/{self.city}/{self.device.get('name', 'device')}.csv"
        if os.path.exists(path):
            print(f"path: {path} is existed")
    ...
    def _place(self):
        ...
        path = f"./libtrajectory/dataset/car/{self.city}/{self.place.get('name', place)}.csv"
        if os.path.exists(path):
            print(f"path: {path} is existed")
