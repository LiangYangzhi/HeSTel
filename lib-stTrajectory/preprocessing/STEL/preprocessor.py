...
class Preprocessor(object):
    def __init__(self, path):
        ...
        self.inter = 60 * 60 * 24 * 7  # 时间分割下的时间间隔阈值
    ...
    def ts2vector(self):
        ...
        tv0 = np.zeros(self.data1.tseg.max() + 1, dtype=int)  # 初始vector，tseg从0开始
        ...
        self.data1["sv"] = self.data1["sv"].map(lambda sv: np.array([int(i) for i in sv]))  # 转成numpy

        logging.info(f"time space vector length: {len(tv0) + svlen}")
        self.data1["tsvec"] = self.data1.apply(lambda row: np.concatenate((row.sv, row.tv)), axis=1)
    ...
    def st2vector(self):
        ...
        sseg = self.data2.sseg.unique().tolist()
        latf = max([int(s.split("-")[0]) for s in sseg]) + 1  # 从0开始标记
        lonf = max([int(s.split("-")[1]) for s in sseg]) + 1  # 从0开始标记
        latv0 = np.zeros(latf, dtype=int)  # 初始vector
        lonv0 = np.zeros(lonf, dtype=int)  # 初始vector
        logging.info(
            f"lat vector length: {len(latv0)}, lon vector length: {len(lonv0)}, space vector length: {len(latv0) + len(lonv0)}")
        ...
        self.data2["tv"] = self.data2["tv"].map(lambda tv: np.array([int(i) for i in tv]))  # 转成numpy

        logging.info(f"space time vector length: {len(latv0) + len(lonv0) + tvlen}")
        self.data2["stvec"] = self.data2.apply(lambda row: np.concatenate((row.sv, row.tv)), axis=1)
    ...
    def ts2graph(self):
        ...
        self.graph1 = self.data1[['tid']].copy()
        self.graph1['node'] = self.graph1['tsvec']
        self.graph1.drop_duplicates(inplace=True)
        self.graph1['node_edge'] = self.graph1['tid'].map(lambda tid: self._ts2ne(tid))
        self.graph1['node'] = self.graph1['node'].map(lambda g: g['node'])
        self.graph1['edge_ind'] = self.graph1['edge_ind'].map(lambda g: g['edge_ind'])
        self.graph1['edge_wei'] = self.graph1['edge_wei'].map(lambda g: g['edge_wei'])
        self.graph1.drop(columns=['node_edge'], inplace=True)
    ...
    def _ts2ne(self, tid):
        ...
        for _, sub_df in dfg:
            ...
            sub_df['node_latlon'] = sub_df.apply(lambda row: {'ind0': row.name, "latlon0": (row.lat, row.lon)}, axis=1)
            df0 = sub_df[['ind0', 'latlon0']].copy()
            df0['flag'] = len(df0)
            df1 = df0.copy()
            df1.rename(columns={'ind0': 'ind1', 'latlon0': 'latlon1'}, inplace=True)
            df = df0.merge(df1, how='outer', on='flag')
            df['edge_ind'] = df.apply(lambda row: [row.ind0, row.ind1], axis=1)
            edge_ind.append(df.edge_ind.tolist())
            df['edge_wei'] = df.apply(lambda row: 1 / geodesic(row.latlon0, row.latlon1).m, axis=1)
            edge_wei.append(df.edge_wei.tolist())
        return {'node': node, 'edge_ind': edge_ind, 'edge_wei': edge_wei}

    def st2graph(self):
        logging.info("space segment and time coordinate --> graph")
        self.graph2 = self.data2[['tid']].copy()  
        self.graph2['node'] = self.graph2['stvec']
        self.graph2.drop_duplicates(inplace=True)                                                      
        self.graph2['node_edge'] = self.graph2['tid'].map(lambda tid: self._st2ne(tid))                 
        self.graph2['node'] = self.graph2['node'].map(lambda g: g['node'])                              
        self.graph2['edge_ind'] = self.graph2['edge_ind'].map(lambda g: g['edge_ind'])                  
        self.graph2['edge_wei'] = self.graph2['edge_wei'].map(lambda g: g['edge_wei'])                  
        self.graph2.drop(columns=['node_edge'], inplace=True)
        logging.info("time segment and space coordinate graph completed")
    ...
    def _st2ne(self, tid):                                                                                             
        ...
        for _, sub_df in dfg:                                                                                          
            if sub_df.shape[0] == 1:                                                                                   
                break                                                                                                  
            # The spatial distance between two nodes                                                                   
            sub_df['node_time'] = sub_df.apply(lambda row: {'ind0': row.name, "time0": row.time}, axis=1)              
            df0 = sub_df[['ind0', 'time0']].copy()                                                                     
            df0['flag'] = len(df0)                                                                                     
            df1 = df0.copy()                                                                                           
            ...
            df['edge_ind'] = df.apply(lambda row: [row.ind0, row.ind1], axis=1)                                        
            edge_ind.append(df.edge_ind.tolist())                                                                      
            df['edge_wei'] = df.apply(lambda row: 1 / abs(row.time0 - row.time1), axis=1)                              
            edge_wei.append(df.edge_wei.tolist())                                                                      
        return {'node': node, 'edge_ind': edge_ind, 'edge_wei': edge_wei}                                              
